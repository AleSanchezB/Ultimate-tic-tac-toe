...
Jugador HaGanado(Jugador** tableroGanados, Jugador quienJugo) {
	...
	//Checa l�neas verticales y horizontales
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
...
void dibujarTablero(sf::RenderWindow& ventana, Celda** tableroMinis, bool** casillasDisponibles, Jugador** tableroGrande)
{
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			cuadrado.setPosition(TAM_CELDA * j * 3 + 10, TAM_CELDA * i * 3 + 5); // Posici�n del cuadrado en la esquina superior izquierda
